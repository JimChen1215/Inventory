...
namespace Verdun.Enterprise.Inventory.Controllers
{
    ...
    {        
        ...
        public IActionResult Delete(int? assetId)
        {
            ...
            vmAsset.AssetTypeAttributeList = _invService.GetAssetAttributeValuesForEdit((int)assetId).ToList();
            _session.SetObjectAsJson("attrList4Delete", vmAsset.AssetTypeAttributeList);
            //MakeAssetTypeSelectList((int)vmAsset.AssetTypeId);
            return View(vmAsset);
        }
        ...
        public IActionResult DeleteConfirmed(int? assetId)
        {
            ...
            List<AssetTypeAttributeViewModel> attrList = _session.GetObjectFromJson<List<AssetTypeAttributeViewModel>>("attrList4Delete"); ;
            if (attrList == null)
            {
                attrList = _invService.GetAssetAttributeValuesForEdit((int)assetId).ToList();
            }
            vmAsset.AssetTypeAttributeList = attrList;
